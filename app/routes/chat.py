...
# -----------------------------
# Send a new message (with FAQ + human request)
# -----------------------------
...
def chat():
    ...
    # 2️⃣ If no FAQ match, check for human/help request
    if not bot_reply_text:
        if "human" in user_message or "help" in user_message:
            bot_reply_text = "An admin will contact you soon!"
            request_human_flag = True
        else:
            bot_reply_text = f"You said '{user_message}'"
...
def admin_human_requests():
    messages = Message.query.filter_by(request_human=True, handled=False).order_by(Message.timestamp.desc()).all()
    return jsonify([{
        "id": m.id,
